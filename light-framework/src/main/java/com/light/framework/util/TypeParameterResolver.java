...
public class TypeParameterResolver {
    ...
    public static List<Class> resolveParameterClassExpand(Type type) {
        List<Class> expandClassList = new ArrayList<>();

        if (type instanceof ParameterizedType) {
            ParameterizedType parameterizedType = (ParameterizedType)type;
            Class<?> targetClass = (Class<?>)parameterizedType.getRawType();
            expandClassList.add(targetClass);
            Type[] typeArguments = parameterizedType.getActualTypeArguments();
            for (Type typeArg : typeArguments) {
                expandClassList.addAll(resolveParameterClassExpand(typeArg));
            }
        } else if (type instanceof GenericArrayType) {
            GenericArrayType genericArrayType = (GenericArrayType)type;
            Type genericComponentType = genericArrayType.getGenericComponentType();
            if (!(genericComponentType instanceof Class)) {
                throw new RuntimeException("不合法的参数声明");
            }
            Object newInstance = Array.newInstance((Class<?>)genericComponentType, 0);
            expandClassList.add(newInstance.getClass());
        } else if (type instanceof WildcardType) {
            throw new RuntimeException("不合法的参数声明");
        } else {
            expandClassList.add((Class<?>)type);
        }
        return expandClassList;
    }
